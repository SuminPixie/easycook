        ...
        <table class="table table-hover notice table-responsive">
          ...
          <tbody>
            <?php
              ...
              $db_b = mysqli_fetch_array($result);

              //내용출력
              while($row = mysqli_fetch_array($result)){
                // 아카데미 이름 가져오기
                $sql_n = "select * from easycook_academy_list where class_no='$row[1]'";
                $result_n = mysqli_query($conn, $sql_n);
                $db_n = mysqli_fetch_array($result_n); //                echo $db_n[1];
                echo "<tr>";
                  echo "<td class='text-center'><a href='./notice_view.php?no=".$db_b[0]."' title='공지사항 수정'>".$row[0]."</a></td>";
                  echo "<td position:relative;'><a href='./notice_view.php?no=".$db_b[0]."' title='공지사항 수정'>
                          <span>".$row[2]."</span>
                          <span class='span_subtitle'>".$db_n[1]."</span></a></td>";
                  echo "<td class='text-center display_none'><a href='./notice_view.php?no=".$db_b[0]."' title='공지사항 수정'>".date("y-m-d",strtotime($row[5]))."</a></td>";
                echo "</tr>";
              };  ?>  
          </tbody>
        </table>
